...
async function listBranches(encodedProjectPath: string, token: string): Promise<string[]> {
  try {
    const url = `${GITLAB_CONFIG.baseUrl}/api/v4/projects/${encodedProjectPath}/repository/branches?per_page=100&private_token=${token}`;
    console.log(`[BRANCHES] Fetching branch list: ${url}`);
    const resp = await fetch(url);
    console.log(`[BRANCHES] HTTP Status: ${resp.status} ${resp.statusText}`);
    if (!resp.ok) {
      try {
        const errorBody = await resp.json();
        console.log(`[BRANCHES] Error:`, JSON.stringify(errorBody));
      } catch (e) {
        console.log(`[BRANCHES] Error parsing failed:`, e);
      }
      return [];
    }
    const branches = await resp.json() as any[];
    const names = branches.map((b: any) => b.name);
    console.log(`[BRANCHES] Found ${names.length} branches:`, names.slice(0, 10));
    return names;
  } catch (error) {
...
async function listRepositoryFiles(repoUrl: string, token: string, path: string = '', branch: string): Promise<{files: string[], triedBranches: string[], actualBranch: string}> {
  try {
    let projectPath = repoUrl;
    if (projectPath.startsWith('http')) {
      projectPath = projectPath.replace(/^https?:\/\/gitlab\.zoesoft\.com\.cn\//, '');
    ...
    const fetchTree = async (dirPath: string, useBranch: string): Promise<any[]> => {
      ...
      console.log(`[DEBUG] Fetching tree: ${url}`);
      const resp = await fetch(url);
      console.log(`[DEBUG] HTTP Status: ${resp.status} ${resp.statusText}`);
      if (!resp.ok) {
        try {
          const errorBody = await resp.json();
          console.log(`[DEBUG] Error response:`, JSON.stringify(errorBody));
        } catch (e) {
          console.log(`[DEBUG] Error response parsing failed:`, e);
        }
        return [];
      }
      const result = await resp.json() as any[];
      console.log(`[DEBUG] Found ${result.length} entries for branch ${useBranch}`);
      return result;
    };
    ...
    const tryBranches = async (dirPath: string): Promise<any[]> => {
      ...
      if (entries.length === 0) {
        const fallbackBranch = branch === 'main' ? 'master' : 'main';
        console.log(`Empty result for ${branch}, trying ${fallbackBranch}...`);
        entries = await fetchTree(dirPath, fallbackBranch);
      ...
      if (entries.length === 0) {
        console.log(`Both ${branch} and master/main returned empty, trying to discover actual branches...`);
        const allBranches = await listBranches(encodedProjectPath, token);
        const candidateBranches = allBranches.filter(
          (b: string) => b !== branch && b !== 'master' && b !== 'main'
        );

        for (const candidate of candidateBranches.slice(0, 5)) {
          console.log(`Trying discovered branch: ${candidate}...`);
          entries = await fetchTree(dirPath, candidate);
          triedBranches.push(candidate);
          if (entries.length > 0) {
            actualBranch = candidate;
            console.log(`✅ Found files on branch: ${candidate}`);
            break;
          }
        }
      }
    ...
    console.log(`Found ${allFiles.length} files total (actual branch: ${actualBranch})`);
    return { files: allFiles, triedBranches: [...new Set(triedBranches)], actualBranch };
  } catch (error) {
    console.error('GitLab list files error:', error);
    return { files: [], triedBranches: [branch], actualBranch: branch };
  }
}
...
export async function getCode(
  ...
  try {
    ...
    if (!targetBranch) {
      targetBranch = GITLAB_CONFIG.defaultBranch || 'master';
    }

    console.log('Using branch:', targetBranch);

    // 确定 Token：使用仓库配置的 Token，或者回退到全局配置
    const repoToken = repoConfig.gitLabToken;
    const globalToken = GITLAB_CONFIG.token;
    const token = repoToken || globalToken;

    console.log('========== [GET_CODE] Token Debug Info ==========');
    console.log('Repository config token exists:', !!repoToken);
    console.log('Global config token exists:', !!globalToken);
    console.log('Using token source:', repoToken ? 'Repository config' : 'Global config');
    console.log('Token value (masked):', token ? token.substring(0, 8) + '****' : 'null');
    ...
    if (filePath) {
      ...
      const { files, triedBranches, actualBranch } = await listRepositoryFiles(repoConfig.repositoryUrl, token, '', targetBranch);
      
      if (files.length === 0) {
        console.log('❌ No files found in repository');
        const repoProjectPath = repoConfig.repositoryUrl.startsWith('http')
          ? repoConfig.repositoryUrl.replace(/^https?:\/\/gitlab\.zoesoft\.com\.cn\//, '').replace(/\//g, '%2F')
          : repoConfig.repositoryUrl.replace(/\//g, '%2F');
        const branchList = await listBranches(repoProjectPath, token);
        const availableBranches = branchList.length > 0 
          ? `\n该仓库实际存在的分支: ${branchList.slice(0, 10).join(', ')}${branchList.length > 10 ? '...' : ''}`
          : '';
        return { 
          success: false, 
          error: `仓库 ${repoConfig.name} 中没有找到代码文件 (已尝试分支: ${triedBranches.join(', ')})${availableBranches}\n请检查仓库默认分支配置或手动指定正确的分支名。` 
        };
      }

      const effectiveBranch = actualBranch || targetBranch;
      const codeFiles = files.slice(0, 30);

      console.log(`✅ Found ${files.length} files (showing ${codeFiles.length}) on branch ${effectiveBranch}`);
      return { 
        success: true, 
        data: {
          serviceName: serviceName || repoConfig.name,
